...
# Using a state dict avoids the "assigned to before global declaration" error
state = {
...
L_EYE, R_EYE = [362, 382, 381, 380, 374, 373, 390, 249, 263, 466, 388, 387, 386, 385, 384, 398], [33, 7, 163, 144, 145,
                                                                                                  153, 154, 155, 133,
                                                                                                  173, 157, 158, 159,
                                                                                                  160, 161, 246]
L_BROW, R_BROW = [276, 283, 282, 295, 285, 300, 293, 334, 296, 336], [46, 53, 52, 65, 55, 70, 63, 105, 66, 107]
...
def handle_click(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN and 0 <= y <= 40:
        ...
        if idx < len(keys): config[keys[idx]] = not config[keys[idx]]
...
try:
    with vision.HandLandmarker.create_from_options(
        ...
        while cap.isOpened():
            ...
            # 1. Pose
            if config["show_body"] and state["latest_pose"] and state["latest_pose"].pose_landmarks:
                p_pts = np.array([[int(lm.x * w), int(lm.y * h)] for lm in state["latest_pose"].pose_landmarks[0]])
            ...
            # 2. Face
            if config["show_face"] and state["latest_face"] and state["latest_face"].face_landmarks:
                f_pts = np.array([[int(lm.x * w), int(lm.y * h)] for lm in state["latest_face"].face_landmarks[0]])
                ...
                if config["show_joints"]:
                    for lm in state["latest_face"].face_landmarks[0]: cv2.circle(output_frame,
                                                                                 (int(lm.x * w), int(lm.y * h)), 1,
                                                                                 (200, 255, 200), -1)

                if config["mouth_click"]:
                    d = np.linalg.norm(np.array(
                        [state["latest_face"].face_landmarks[0][13].x, state["latest_face"].face_landmarks[0][13].y]) -
                                       np.array([state["latest_face"].face_landmarks[0][14].x,
                                                 state["latest_face"].face_landmarks[0][14].y]))
                    if d > 0.04:
                        if not state["mouth_is_open"]:
                            pyautogui.click();
                            state["mouth_is_open"] = True
                    else:
                        state["mouth_is_open"] = False

            # 3. Hands
            if config["show_hand"] and state["latest_hand"]:
                for idx, hand_lms in enumerate(state["latest_hand"].hand_landmarks):
                    ...
                    if config["mouse_active"] and idx == 0:
                        off = (smoothed_hands[0][8] - state["hand_zero_pt"]) * 2.5
