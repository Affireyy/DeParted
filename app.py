...
smoothed_hands = {}
prev_time = time.time()
mouth_is_open = False  # Debounce for clicking
...
with vision.HandLandmarker.create_from_options(
    ...
    while cap.isOpened():
        ...
        # 2. Face (Restored & Detailed Skeletons)
        if config["show_face"] and latest_face and latest_face.face_landmarks:
            ...
            # Mouth Click Logic (Indices 13 and 14 are top/bottom inner lip center)
            if config["mouse_active"] and config["mouth_click"]:
                ...
                if dist > 0.04:  # Threshold for open
